...
```sql
SELECT symbole, SUM(nombreOccurrences) /
    (SELECT SUM(nombreCaracteres) FROM corpus WHERE langue = "Français")
FROM occurences JOIN corpus JOIN caractere ON
caractere.idCar = occurences.ifCar AND corpus.idLivre = occurences.idLivre
