...
On peut éviter de réaliser une analyse de la chaîne de caractères à coder en exploitant des données statistiques disponibles. La numérisation de larges corpus littéraires a permis la création de bases de données contenant des informations relatives au nombre d’occurrences des différents caractères alphanumériques dans diverses langues.

Nous allons supposer disposer d’une base de données constituée de trois tables (les clefs primaires sont soulignées) :
- caractere(#underline[idCar], symbole, typeCaractere, codeHTML) ;
- corpus(#underline[idLivre], titre, auteur, annee, nombreCaracteres, langue) ;
- occurrences(#underline(stroke: (dash: "dashed"))[idCar], #underline(stroke: (dash: "dashed"))[idLivre], nombreOccurrences) ;
...
#question[
    Déterminer le caractère qui suit 'ad' dans la chaîne codée par `x=0.123` en
